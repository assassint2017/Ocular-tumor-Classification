...
for item in os.listdir(yz_dir):
    ...
    try:
        ...
        lesion = sitk.ReadImage(os.path.join(yz_dir, item, 'mask_T2.mha'))
    ...
    print(T2.shape)
    print(lesion.shape)
    if T2.shape[1:] != lesion.shape[1:]:
        print('Axial not match!')
    if T2.shape[0] != lesion.shape[0]:
...
for item in os.listdir(lbl_dir):
    ...
    try:
        ...
        lesion = sitk.ReadImage(os.path.join(lbl_dir, item, 'mask_T2.mha'))
        lesion = sitk.GetArrayFromImage(lesion)
...
# 发现mask_C分割金标准和原始T1C数据的维度有一些是不一致的(轴向和纵向都会出现不一致),轴向14例,纵向18例
# 纵向一般缺少一张或者两张最多是三张slcie,轴向是514*512 512*512
# 所有数据读取是没有问题的
# 下面是病灶平均大小
# 99.16883116883118
# 76.27922077922078
